...
logger = logging.getLogger("CryptoMining_AILEE_Governance")

class CryptoMiningGovernanceWrapper:
    ...
    def execute_hashrate_tuning(self, mining_data: dict, peer_miners: list = None):
        """
        Executes hashrate_tuning with AILEE governance.
        """
        raw_hashrate, raw_confidence = self.legacy_ai.predict_tuning(mining_data)

        if not self.ailee_enforced:
            logger.warning(f"LEGACY BYPASS: Executing unverified AI command: {raw_hashrate}")
            self._actuate_rig(raw_hashrate)
            return raw_hashrate

        peer_values = [p.get_prediction() for p in peer_miners] if peer_miners else []
        result = self.trust_layer.process(
            raw_value=raw_hashrate, raw_confidence=raw_confidence, peer_values=peer_values,
            context={"operation": "hashrate_tuning", "mining_data": mining_data}
        )

        if result.safety_status == "OUTRIGHT_REJECTED":
            logger.error(f"AILEE INTERVENTION: Unsafe tuning rejected. Using safe fallback: {result.value}")
            self._actuate_rig(result.value)
            return result.value
    ...
    def _actuate_rig(self, final_hashrate: float):
