...
logger = logging.getLogger("Grids_AILEE_Governance")

class GridsGovernanceWrapper:
    ...
    def execute_load_dispatch(self, grid_data: dict, peer_nodes: list = None):
        """
        Executes load_dispatch with AILEE governance.
        """
        raw_load, raw_confidence = self.legacy_ai.predict_dispatch(grid_data)

        if not self.ailee_enforced:
            logger.warning(f"LEGACY BYPASS: Executing unverified AI command: {raw_load}")
            self._actuate_grid(raw_load)
            return raw_load

        peer_values = [p.get_prediction() for p in peer_nodes] if peer_nodes else []
        result = self.trust_layer.process(
            raw_value=raw_load, raw_confidence=raw_confidence, peer_values=peer_values,
            context={"operation": "load_dispatch", "grid_data": grid_data}
        )

        if result.safety_status == "OUTRIGHT_REJECTED":
            logger.error(f"AILEE INTERVENTION: Unsafe load dispatch rejected. Using safe fallback: {result.value}")
            self._actuate_grid(result.value)
            return result.value
    ...
    def _actuate_grid(self, final_load: float):
