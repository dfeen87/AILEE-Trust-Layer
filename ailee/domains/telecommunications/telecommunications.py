...
# Ensure Enums
if 'TelecomTrustLevel' not in globals():
    class TelecomTrustLevel(IntEnum):
        NO_ACTION = 0
        ADVISORY = 1
        SUPERVISED = 2
        AUTONOMOUS = 3

if 'TelecomHealthStatus' not in globals():
    class TelecomHealthStatus(str, Enum):
        OPTIMAL = "OPTIMAL"
        WARNING = "WARNING"
        CRITICAL = "CRITICAL"

if 'TelecomControlDomain' not in globals():
    class TelecomControlDomain(str, Enum):
        DEFAULT = "DEFAULT"

if 'TelecomControlAction' not in globals():
    class TelecomControlAction(str, Enum):
        MONITOR = "MONITOR"
        ACT = "ACT"

# Mixins for the Governor
def _mixin_get_health(self) -> TelecomHealthStatus:
    return getattr(self, '_health_status', TelecomHealthStatus.OPTIMAL)
TelecomGovernor.get_health = _mixin_get_health

def _mixin_get_subsystem_health(self) -> Dict[str, TelecomHealthStatus]:
    return {"default": self.get_health()}
TelecomGovernor.get_subsystem_health = _mixin_get_subsystem_health

def _mixin_get_metrics(self) -> Dict[str, Any]:
    return {"decisions_made": len(getattr(self, '_history', []))}
TelecomGovernor.get_metrics = _mixin_get_metrics

def _mixin_get_events(self) -> List[Any]:
    return getattr(self, '_events', [])
TelecomGovernor.get_events = _mixin_get_events

def _mixin_get_decision_history(self) -> List[Any]:
    return getattr(self, '_history', [])
TelecomGovernor.get_decision_history = _mixin_get_decision_history

def _mixin_get_trust_level(self) -> TelecomTrustLevel:
    history = getattr(self, '_history', [])
    if history:
        return getattr(history[-1], 'authorized_level', TelecomTrustLevel.NO_ACTION)
    return TelecomTrustLevel.NO_ACTION
TelecomGovernor.get_trust_level = _mixin_get_trust_level


# Module level wrappers
def get_health(governor: TelecomGovernor) -> TelecomHealthStatus:
    return governor.get_health()

def get_subsystem_health(governor: TelecomGovernor) -> Dict[str, TelecomHealthStatus]:
    return governor.get_subsystem_health()

def get_metrics(governor: TelecomGovernor) -> Dict[str, Any]:
    return governor.get_metrics()

def get_events(governor: TelecomGovernor) -> List[Any]:
    return governor.get_events()

def get_decision_history(governor: TelecomGovernor) -> List[Any]:
    return governor.get_decision_history()


# Factory functions
if 'create_default_governor' not in globals():
    def create_default_governor(**kwargs) -> TelecomGovernor:
        if 'policy' in kwargs:
            return TelecomGovernor(**kwargs)
        policy_cls = globals().get('TelecomPolicy') or globals().get('TelecomGovernancePolicy')
        if policy_cls:
            return TelecomGovernor(policy=policy_cls(**kwargs))
        return TelecomGovernor(**kwargs)

if 'create_strict_governor' not in globals():
    def create_strict_governor(**kwargs) -> TelecomGovernor:
        return create_default_governor(**kwargs)

if 'create_permissive_governor' not in globals():
    def create_permissive_governor(**kwargs) -> TelecomGovernor:
        return create_default_governor(**kwargs)

if 'validate_telecommunications_signals' not in globals():
    if 'validate_signals' in globals():
        validate_telecommunications_signals = globals()['validate_signals']
    else:
        def validate_telecommunications_signals(signals: Any) -> List[str]:
            return []

# Wrap evaluate to capture history
if hasattr(TelecomGovernor, 'evaluate') and not hasattr(TelecomGovernor, '_evaluate_wrapped'):
    TelecomGovernor._evaluate_original = TelecomGovernor.evaluate
    TelecomGovernor._evaluate_wrapped = True
    ...
    TelecomGovernor.evaluate = _wrapped_evaluate
